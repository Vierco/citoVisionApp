...
2. Leer documentación derivada o relacionada según la tarea:
   ...
   - `~/.claude/skills/<nombre_del_skill>/SKILL.md` (skills a nivel de usuario, fuera del repositorio): procedimientos operativos específicos.
...
Las skills viven **a nivel de usuario**, en `~/.claude/skills/<nombre_del_skill>/SKILL.md`, fuera del repositorio: Claude Code las descubre de forma nativa desde ahí para todos los proyectos. **No están versionadas en este repo** (lo estuvieron hasta septiembre de 2026, para que el tribunal del TFM pudiera consultarlas). **Excepciones externas:** `android-official` es una colección de Google (Apache 2.0) que se instala aparte con Android CLI, y `apple-design` es la skill de referencia de las HIG de Apple de [tzzs/apple-design-skill](https://github.com/tzzs/apple-design-skill) (MIT).
...
| Apple Design (HIG) | `~/.claude/skills/apple-design` — _externa: [tzzs/apple-design-skill](https://github.com/tzzs/apple-design-skill), MIT_ | `SKILL.md` + `references/` | **Solo para la parte nativa de iOS**: la barra de pestañas nativa y sus diálogos (ADR-0008), código SwiftUI/UIKit en `iosApp/`, Liquid Glass, safe areas, SF Symbols, icono de iOS, o revisar si una pantalla se siente nativa en iPhone. **No** para la UI compartida en Compose, que sigue `DESIGN.md` y Material 3. |
